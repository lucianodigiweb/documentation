...
subscriptions with recurring payments with their favorite payment methods such as **credit cards**.
...
.. image:: payment_providers/online-payment.png
   :alt: Pay online and select your preferred payment method.
...
.. note::
   Each provider has its own specific configuration flow, depending on :ref:`which feature is
   available <payment_providers/online_providers>`.
...
#. Go to the payment provider's website, create an account, and make sure you have the credentials
   requested for third-party use. These are necessary for Odoo to be able to communicate with the
   payment provider.
#. Configure the :guilabel:`Credentials` tab; the fields available depend on the payment provider.
...
payment provider form to :guilabel:`Test mode`, then enter your provider's test/sandbox credentials
...
If you want to further configure payment methods, e.g. disable tokenization or further restrict
the supported countries/currencies, go to :menuselection:`Accounting --> Configuration -->
...
activate the :ref:`developer mode <developer-mode>`. Select the :guilabel:`Configuration` tab to
adapt the features.
...
.. warning::
   ...
   - Modifications to the payment method's configuration only work to the extent of the method
     and provider's capabilities. For example, you can disable :ref:`tokenization
     <payment_providers/features/tokenization>` or remove :ref:`supported countries
     <payment_providers/currencies_countries>` for a payment method if you wish to. However, adding
     countries for a method that is only supported in on country or enabling tokenization for a
     method linked to a provider that does not support it will not produce the intended results.
...
can choose to save their card details for later. To enable this feature, go to the
...
payments, without the customer having to enter their card details again. This is particularly useful
for the eCommerce conversion rate and for subscriptions that use recurring payments.
...
You can also void the authorization to release the reserved funds; this is equivalent to a regular
cancellation. Capturing payments manually is useful in many situations:
...
A payment journal must be defined for the payment provider to record the payments on an
**outstanding account**. By default, the :guilabel:`Bank` journal is added as the payment journal
for all payment providers. To modify it, go to the :guilabel:`Configuration` tab of the selected
payment provider and select another :guilabel:`Payment journal`.
...
go directly on your bank account and follow the usual :doc:`reconciliation
<accounting/bank/reconciliation>` workflow and those coming from :ref:`online payment providers
<payment_providers/online_providers>` that are third-party services and require you to follow
another accounting workflow. For these payments, you need to consider how you want to record your
payments' journal entries. We recommend you to ask your accountant for advice.

By default, the :guilabel:`Bank Account` defined for the :ref:`payment journal
<payment_providers/journal>` is used, but you can also define an **Outstanding account** for each
payment provider to separate the provider's payments from other payments in the
:guilabel:`Incoming Payments` and :guilabel:`Outgoing Payments` tabs.
